      ...
      <div className={style.bot}>
        ...
        {showIntro && (
          <div className={style.intro}>
            <img className={style.popupimage} src={BotIcon} alt="" />
            <p style={{ fontSize: "smaller" }}>{botconfig.defaultmessages}</p>
            <button
              onClick={() => {
                setShowIntro((prev) => {
                  const newValue = !prev;
                  localStorage.setItem("showintro", newValue ? "1" : "0");
                  return newValue;
                });
              }}
              className={style.introclear}
            >
              X
            </button>
          </div>
